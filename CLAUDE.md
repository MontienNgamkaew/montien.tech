...
4. **pnp-go ใช้ mPDF (Composer):** `pnp-go/vendor/` **ถูก commit ขึ้น git แล้ว** → `git pull`/checkout มี mPDF ทันที **ไม่ต้องรัน `composer install` บน host**
   - เหตุผล: Hostinger ทำ fresh checkout ตอน deploy → ลบไฟล์ที่ไม่ได้อยู่ใน git (vendor ที่ติดตั้งด้วย composer มือจะหาย → PDF พัง `Class "Mpdf\…" not found`)
   - ตัดฟอนต์ที่ mPDF แถม (`vendor/mpdf/mpdf/ttfonts/` ~88MB) ออกจาก git (ดู `pnp-go/.gitignore`) — ระบบใช้ THSarabunNew จาก `pnp-go/storage/fonts/` (track ใน git) แทน
   - ถ้าเพิ่ม/อัปเดต dependency: รัน `composer install` ใน local แล้ว `git add pnp-go/vendor` commit ขึ้นไป (อย่าลืมว่า ttfonts ถูก ignore)
