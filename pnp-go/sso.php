...
if ($isPortalAdmin !== 1) {
    $appStatus = 'active';
    try {
        $portalDb = Database::portalConnection();
        $stmtStatus = $portalDb->prepare("SELECT status FROM app_status WHERE app_id = 'pnp-go' LIMIT 1");
        $stmtStatus->execute();
        ...
        $appStatus = 'active'; // ถ้าตรวจสถานะไม่ได้ ปล่อยให้เข้าได้ตามปกติ
    }
    ...
    if ($appStatus === 'disabled' || $appStatus === 'coming_soon') {
        ...
        $desc  = $isSoon
            ? 'ระบบขออนุญาตใช้รถยนต์อยู่ระหว่างการพัฒนา จะเปิดให้บริการในเร็ววัน ขออภัยในความไม่สะดวก'
        ...
        echo '<body class="bg-light d-flex align-items-center justify-content-center" style="min-height:100vh;">';
        echo '<div class="card shadow-sm p-4 text-center" style="max-width:480px;border-radius:16px;">';
        echo '<div class="fs-1 mb-3">' . $icon . '</div>';
        echo '<h4 class="text-dark mb-3">' . $head . '</h4>';
        echo '<p class="text-muted mb-4">' . $desc . '</p>';
        echo '<a href="/pnp-portal/" class="btn btn-outline-secondary w-100 py-2" style="border-radius:10px;">กลับสู่หน้าแรกพอร์ทัลกลาง</a>';
        echo '</div></body></html>';
        exit;
    }
}
